    ...
    public void onPlayerStoppedUsing(ItemStack stack, World worldIn, EntityLivingBase entityLiving, int timeLeft)
    {
        if (entityLiving instanceof EntityPlayer)
        {
            ...
            if (!itemstack.isEmpty() || flag)
            {
                ...
                if ((double)f >= 0.1D)
                {
                    boolean flag1 = entityplayer.capabilities.isCreativeMode || (itemstack.getItem() instanceof ItemSpecialArrow && ((ItemSpecialArrow) itemstack.getItem()).isInfinite(itemstack, stack, entityplayer));
